...
function objToArray(obj, nameOfKeyInNewArrayObj, typecast = o => o) {
  return Object.keys(obj)
    .map(key => {
      const element = {...obj[key]}
      element[nameOfKeyInNewArrayObj] = typecast(key)
      return element
    })
}
...
async function getAll() {
  ...
  const reports = allFileContents
    .map(o => o.reports)
    .flatMap(o => objToArray(o, 'time', Number))
  return reports
}

async function get(user, time) {
...
function add(user, time, report, raw) {
  ...
  const isNew = !current.reportsRaw[time]
  if (isNew) {
    current.reports[time] = report
    current.reportsRaw[time] = raw
    save(user, current)
  }
  return isNew
}

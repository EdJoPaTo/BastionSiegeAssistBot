...
function createPlayerStatsString(allBattlereports, stats) {
  ...
  if (stats.attacksWithoutLossPercentage >= 0) {
    text += `\nFarm: ${Math.round(stats.attacksWithoutLossPercentage * 100)}% (100% is best for easy loot)`
  }
  ...
  if (stats.smallestSuccessfulArmyLately && stats.smallestSuccessfulArmyLately !== Number.POSITIVE_INFINITY) {
    text += `\nSmallest army that won: ${stats.smallestSuccessfulArmyLately}${emoji.army}`
  }
  if (stats.strongestArmyThatLost && stats.strongestArmyThatLost !== Number.NEGATIVE_INFINITY) {
    text += `\nStrongest army that lost: ${stats.strongestArmyThatLost}${emoji.army}`
  }
...
function createPlayerStatsShortString(allBattlereports, stats) {
  ...
  if (stats.attacksWithoutLossPercentage >= 0) {
    text += ` Farm: ${Math.round(stats.attacksWithoutLossPercentage * 100)}%.`
  }
  if (stats.loot.amount > 0) {

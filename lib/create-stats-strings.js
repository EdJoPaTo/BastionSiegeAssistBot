...
function createPlayerStatsString(allBattlereports, stats) {
  ...
  text += createStatsBarLine(stats.armyAttack, emoji.army)
  text += createStatsBarLine(stats.armyDefense, emoji.wall)
...
function createStatsBarLine(data, emoji) {
  let text = ''
  ...
  if (isFinite(data.max)) {
    text += data.max
    text += ' ('
    text += `${Math.round(data.maxPerventLost * 100)}% lost`
    text += '; '

...
function createPlayerStatsString(allBattlereports, stats) {
  ...
  if (stats.attacksWithoutLossPercentage >= 0) {
    text += `\nFarm: ${stats.attacksWithoutLossPercentage * 100}% (100% is best for easy loot)`
  }
  if (stats.loot.amount > 0) {
    text += '\n' + createSumAverageAmountString(stats.loot, 'Loot', emoji.gold)
  }

  if (stats.smallestSuccessfulArmyLately && stats.smallestSuccessfulArmyLately !== Number.POSITIVE_INFINITY) {
    text += `\nSmallest army that won: ${stats.smallestSuccessfulArmyLately}`
  }
  if (stats.strongestArmyThatLost && stats.strongestArmyThatLost !== Number.NEGATIVE_INFINITY) {
    text += `\nStrongest army that lost: ${stats.strongestArmyThatLost}`
  }
...
function createPlayerStatsShortString(allBattlereports, stats) {
  ...
  if (stats.immune) {
    text += 'This player is an active user of this bot. You will not get any information about him. Do the same and get the same kind of protection. 🛡💙'
    return text
  }

  text += ` ${stats.battlesObserved} battle${stats.battlesObserved > 1 ? 's' : ''} observed (${stats.winsObserved}|${stats.lossesObserved}).`
  if (stats.attacksWithoutLossPercentage >= 0) {
    text += ` Farm: ${stats.attacksWithoutLossPercentage * 100}%.`
  }
  if (stats.loot.amount > 0) {
    text += ` Loot (${stats.loot.amount}): ~${formatNumberShort(stats.loot.avg)}${emoji.gold}.`
  }

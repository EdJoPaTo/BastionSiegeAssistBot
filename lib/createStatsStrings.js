...
const { calcGoldCapacity, calcGoldIncome, calcBuildingCost, calcProduction, calcProductionFood, calcStorageCapacity, calcStorageLevelNeededForUpgrade, calcTownhallLevelNeededForUpgrade, calcMinutesNeeded, calcMinutesNeededToFillStorage } = require('../lib/siegemath')
...
function createFillTimeStatsString(buildings, resources) {
  ...
  const fillRessources = { wood: storageCapacity, stone: storageCapacity, food: storageCapacity }
  ...
  text += '\n'
  text += `${emoji.storage} fill in ${formatTime(minutesNeededToFillStorage)} (${createNeededMaterialStatString(fillRessources, resources)})\n`

...
async function add(timestamp, battle) {
  ...
  return Promise.all(inlineMessagesToUpdate.map(({inlineMessageId, isAttack}) =>
    telegram.editMessageText(undefined, undefined, inlineMessageId, createWarStats(timestamp, battle, isAttack ? battle.attack[0] : battle.defence[0]), Extra.markdown())
      .catch(error => console.error('could not update war', error))
  ))
}

...
function getBattlereport(content) {
  ...
  const reward = regexHelper.getNumber(content, `Your reward is (\\d+)${emoji.gold}`) || 0
  ...
  if (winners) {
    ...
    const enemy = regexHelper.get(content, /battle with (?:{([^}]+)})?(?:\[([^\]]+)\])?(.+) complete/, 3)
    battlereport.enemies = [enemy]
  }

  const soldiersMatch = new RegExp(`(\\d+)${emoji.army} of (\\d+)${emoji.army}`).exec(content)
  const noneOf = regexHelper.getNumber(content, `None of the (\\d+)${emoji.army}`)
  const withoutLoss = regexHelper.getNumber(content, `(\\d+)${emoji.army} without a loss`)
  if (soldiersMatch) {

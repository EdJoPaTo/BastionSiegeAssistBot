...
function detectType(content) {
  ...
  if (lines[0].includes('Your ally ') &&
      lines[0].includes(' attacked ') &&
      lines[0].includes(' help ')) {
    return 'allianceBattleStart'
  }

  if (lines[0].includes('\'s army joined the ')) {
  ...
  if (lines[0].includes(' joined the siege ')) {
    if (lines[0].includes(' The leader of your alliance ')) {
      return 'castleSiegeLeaderJoined'
    }

    if (lines[0].startsWith('The alliance ')) {
      return 'castleSiegeAllianceJoined'
    }

    if (lines[0].includes(' Your army ') || lines[0].includes(' Your alliance ')) {
...
function getAllianceBattleStart(content) {
  ...
  const ally = parsePlayer(defenceMatch ? defenceMatch[1] : attackMatch[1])
  const enemy = parsePlayer(defenceMatch ? defenceMatch[2] : attackMatch[2])
  return {allianceBattleStart: {
    attack: !defenceMatch,
    ally: ally.name,
    enemy: enemy.name
...
function getAllianceBattleSupport(content) {
  const namePart = regexHelper.get(content, /🕊(.+)'s army joined the /)
  const {name} = parsePlayer(namePart)
  return {allianceBattleSupport: {
    player: name
  }}
...
function getCastleSiegeAllianceJoined(content) {
  const namePart = regexHelper.get(content, /The alliance (.+) joined the siege on /)

...
function getAllianceBattleStart(content) {
  ...
  const ally = gamescreenName.parse(defenceMatch ? defenceMatch[1] : attackMatch[1])
  const enemy = gamescreenName.parse(defenceMatch ? defenceMatch[2] : attackMatch[2])
  return {allianceBattleStart: {
    attack: !defenceMatch,
    ally: ally.name,
    enemy: enemy.name
...
function getCastleSiegePlayerJoined(content) {
  ...
  const {name, alliance} = gamescreenName.parse(namePart)
  return {castleSiegePlayerJoined: {
    alliance,
    player: name
  }}
...
function getCastleSiegeAllianceJoined(content) {
  ...
  const {alliance} = gamescreenName.parse(namePart)
  return {castleSiegeAllianceJoined: {
    alliance
  }}
}

const {parsePlayer} = require('bastion-siege-logic')
...
function detectType(content) {
  ...
  if (lines[0].includes(' союзник') &&
      lines[0].includes(' атаковал ') &&
      lines[0].includes(' Ты можешь ')) {
    return 'allianceBattleStart'
  }

  if (lines[0].includes(' присоединилась к ') && !lines[0].includes('Твоя армия ')) {
  ...
  if (lines[0].includes('окончено. Осада вот-вот начнется. В ней примут участие')) {
    return 'castleSiegeParticipants'
  }

  if (lines[0].startsWith('⚔️🏰 Твоя армия присоединилась к осаде на')) {
    return 'castleSiegeYouJoined'
  }

  if (lines[0].includes(' присоединился к осаде на ')) {
    if (lines[0].includes(' Лидер твоего альянса ')) {
      return 'castleSiegeLeaderJoined'
    }

    if (lines[0].startsWith('Альянс ')) {
      return 'castleSiegeAllianceJoined'
    }

    return 'castleSiegePlayerJoined'
  }
...
function getScreenInformation(type, content) {
  switch (type) {
    ...
    case 'castleSiegeAllianceJoined': return getCastleSiegeAllianceJoined(content)
    case 'castleSiegeLeaderJoined': return getCastleSiegePlayerJoined(content)
    case 'castleSiegeParticipants': return getCastleSiegeParticipants(content)
    case 'castleSiegePlayerJoined': return getCastleSiegePlayerJoined(content)
    case 'castleSiegeYouJoined': return {castleSiegeYouJoined: {}}
...
function getRawBattlereport(content) {
  ...
  const enemy = parsePlayer(fullEnemy)
  ...
  if (noneOf !== undefined) {
    ...
    console.error('soldiers unknown', content)
...
function getAttackScout(content) {
  ...
  const {name} = parsePlayer(namePart)
...
function getAllianceBattleStart(content) {
  ...
  const ally = parsePlayer(regexHelper.get(content, regex, 1))
  const enemy = parsePlayer(regexHelper.get(content, regex, 2))
  return {allianceBattleStart: {
    attack,
    ally: ally.name,
    enemy: enemy.name
...
function getAllianceBattleSupport(content) {
  const namePart = regexHelper.get(content, /🕊..?Армия (.+) присоединилась к /)
  const {name} = parsePlayer(namePart)
  return {allianceBattleSupport: {
    player: name
  }}
...
function getCastleSiegePlayerJoined(content) {
  const emojis = emoji.regexArmy + emoji.castle
  const regex = `${emojis}(?: Лидер твоего альянса)? (.+) присоединился к осаде на `
  const namePart = regexHelper.get(content, regex)
  const {name, alliance} = parsePlayer(namePart)
  return {castleSiegePlayerJoined: {
    alliance,
    player: name
  }}
...
function getCastleSiegeAllianceJoined(content) {
  const namePart = regexHelper.get(content, /Альянс (.+) присоединился к осаде на /)
  const {alliance} = parsePlayer(namePart)
  return {castleSiegeAllianceJoined: {
    alliance
  }}
}

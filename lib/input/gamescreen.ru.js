...
function detectType(content) {
  ...
  if (lines[0].indexOf('Победы') >= 0) {
    return 'war'
  }

  if (lines[0].indexOf('‼️Битва с ') >= 0) {
    return 'battlereport'
  }
  ...
  if ((content.indexOf('Продлится до: ') >= 0 || content.indexOf('Продлится еще: ') >= 0) &&
...
function getRawBattlereport(content) {
  ...
  const fullEnemy = regexHelper.get(content, /Битва с (?:альянсом )?(.+) окончена/)

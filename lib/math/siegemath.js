...
function calcWallArcherCapacity(wallLevel) {
  return wallLevel * 10
}
...
function calcWallRepairCost(wallLevel) {
...
function calcBuildingCostPerWinchanceSolo(building, currentBuildingLevel) {
  const winChance = BATTLE_ODDS.SOLO[building]
...
function calcBuildingCostPerWinchanceAlliance(building, currentBuildingLevel) {
  const winChance = BATTLE_ODDS.ALLIANCE[building]
  return calcBuildingCostPerWinchance(building, currentBuildingLevel, winChance)
}

function calcBuildingCostPerWinchance(building, currentBuildingLevel, winChance = 0) {
  const buildingLevelsRequired = 1 / winChance
  ...
  const resultCost = {}
  for (const key of Object.keys(buildingCostComplete)) {
    resultCost[key] = buildingCostComplete[key] + (buildingCostTopoff[key] * partialRequirementPercent)
  }

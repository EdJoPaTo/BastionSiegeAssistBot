...
function createBattleStatsString(stats, selector) {
  ...
  text += createBattleStatsStringPerAlliance(stats.mystics, 'Mystics', emoji[selector], true)

  text += createBattleStatsStringPerAlliance(stats.rewardAttackWon, 'Successful Attacks', emoji[selector], true)
  text += createBattleStatsStringPerAlliance(stats.rewardDefenseWon, 'Successful Defenses', emoji[selector], true)
  text += createBattleStatsStringPerAlliance(stats.rewardAttackLost, 'Lost Attacks', emoji[selector], true)
  text += createBattleStatsStringPerAlliance(stats.rewardDefenseLost, 'Lost Defenses', emoji[selector], true)
...
function createRanking(data, key, title, forceIncludeName) {
  const entries = data
    ...
    .filter((o, i) => {
      if (o.playername === forceIncludeName) {
        return true
      }

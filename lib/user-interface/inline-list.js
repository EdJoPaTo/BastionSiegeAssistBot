...
function createList(creatorId, now) {
  const list = lists.getList(creatorId, now)
  const {participants} = list

  const entries = Object.keys(participants)
    .map(o => Number(o))
    .map(id => getEntryInformation(id, userSessions.getUser(id)))
  ...
  const buttonPrefix = `inlineList:${creatorId}:`

...
function createMultiplePlayerStatsStrings(stats) {
  const buttons = stats
    .map(o => o.player)
    .map(o => Markup.switchToChatButton(`Share ${o}…`, o))

  const statsStrings = stats
    .map(o => createPlayerStatsString(o))

  return {buttons, statsStrings}
}
...
module.exports = {
  createMultiplePlayerStatsStrings,
